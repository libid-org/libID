...
by libid-deploy 0.6.0), the released notary, libID-bridge-rs and keeper
...
The bridge repository is now [`libID-bridge-rs`](https://github.com/libid-org/libID-bridge-rs).
This legacy harness intentionally retains its existing Compose service name
and 0.2.2 image pin. That release implements the older proof-service API;
changing its image to the current browser-ceremony bridge would break the
harness. The repository rename does not change that published image.
